...
const TABLE_SVG = `<svg width="920" height="590" viewBox="0 0 920 590" xmlns="http://www.w3.org/2000/svg" font-family="-apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif">
  <defs>
...
const Check = ({ blue }: { blue?: boolean }) => (
  <svg width="18" height="18" viewBox="0 0 20 20" fill="none" aria-hidden="true">
    <circle cx="10" cy="10" r="9" fill={blue ? "#EAF2FF" : "#F0F0F3"} stroke={blue ? "#B9D4FF" : "#DFDFE4"}/>
    {blue
      ? <path d="M6.5 10l2.5 2.5 4.5-5" stroke="#0071E3" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
      : <path d="M7 10l2 2 4-4" stroke="#0071E3" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
    }
  </svg>
)
...
const Cross = () => (
  <svg width="18" height="18" viewBox="0 0 20 20" fill="none" aria-hidden="true">
    <circle cx="10" cy="10" r="9" fill="#F9F9F9" stroke="#DFDFE4"/>
    <path d="M7 7l6 6M13 7l-6 6" stroke="#C7C7CC" strokeWidth="1.6" strokeLinecap="round"/>
  </svg>
)
      ...
      <div className="mx-auto w-full max-w-[1200px] px-6 md:px-12">
        ...
        <div className="reveal hidden md:flex mb-12 justify-center">
          <div
            ...
            dangerouslySetInnerHTML={{ __html: TABLE_SVG }}
          />
        </div>

        {/* Mobile: HTML table */}
        <div className="reveal md:hidden mb-10 overflow-hidden rounded-3xl border border-[var(--border)] shadow-[0_8px_32px_rgba(0,0,0,0.10)]" style={{ background: "#fff" }}>
          {/* Window chrome */}
          <div className="flex items-center gap-2 border-b border-[#D8DADF] bg-[#F5F5F7] px-4 py-3">
            <span className="h-3 w-3 rounded-full bg-[#FF5F57]" />
            <span className="h-3 w-3 rounded-full bg-[#FEBC2E]" />
            <span className="h-3 w-3 rounded-full bg-[#28C840]" />
            <span className="ml-2 text-[12px] text-[#6E6E73]">DiskCleaner</span>
          </div>

          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="border-b border-[#D8DADF]">
                <th className="py-3 pl-4 pr-2 text-[12px] font-semibold text-[#1D1D1F] w-[44%]">Feature</th>
                <th className="py-3 px-2 text-center text-[12px] font-bold text-[#0071E3] w-[19%] bg-[#EAF2FF]">Disk<br/>Cleaner</th>
                <th className="py-3 px-2 text-center text-[11px] font-semibold text-[#1D1D1F] w-[19%] leading-tight">Sub.<br/>Cleaner</th>
                <th className="py-3 px-2 text-center text-[11px] font-semibold text-[#1D1D1F] w-[18%] leading-tight">Free<br/>Cleaner</th>
              </tr>
            </thead>
            <tbody>
              {ROWS.map((row, i) => (
                <tr key={i} className="border-b border-[#F0F0F3]">
                  <td className="py-3 pl-4 pr-2 text-[12px] leading-[1.35] text-[#1D1D1F]">{row.f}</td>
                  <td className="py-3 px-2 text-center bg-[#EAF2FF]">
              ...
              {/* Price row */}
              <tr>
                <td className="py-3 pl-4 pr-2 text-[12px] font-bold text-[#1D1D1F]">Price</td>
                <td className="py-3 px-2 text-center bg-[#EAF2FF] text-[13px] font-bold text-[#0071E3]">$9.99</td>
                <td className="py-3 px-2 text-center text-[11px] font-medium text-[#6E6E73]">$35/yr</td>
                <td className="py-3 px-2 text-center text-[11px] font-medium text-[#6E6E73]">Free</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", textAlign: "center" }}>
          <p style={{ fontSize: "clamp(16px,2.5vw,21px)", fontWeight: 400, lineHeight: 1.55, letterSpacing: ".011em", color: "#606f7f", margin: 0 }}>
